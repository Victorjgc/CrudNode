const moviestext = [{ name: 'BatmanVSuperman', id: 1, likes: 0 }, { name: 'The Last Samurai', id: 2, likes: 1 }];
    ...
    return new Promise((resolve, reject) => {
      MongoClient.connect(MONGO_URL, (err, client) => {
        if (!err) {
          ...
          moviesCollection.findOne({ _id:   findmovie }).then(movie => resolve(movie)).catch(errorDelete => reject(errorDelete));
        } else {
          reject(err);
        }
      });
...
export function createMovieMongo(moviemongo) {
    return new Promise((resolve, reject) => {
      let movieToInsert = { ... moviemongo, likes: 0,  created: new Date(), updated: new Date() };
      MongoClient.connect(MONGO_URL, (err, client) => {
        if (!err) {
          const db = client.db('movies');
          const moviesCollection = db.collection('movies');
